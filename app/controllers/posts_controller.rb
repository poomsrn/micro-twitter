class PostsController < ApplicationController
  ...
  # GET /posts/1/edit
  def edit
    if @post.user_id != get_session
      redirect_to main_path, notice: "You are not allow to access."
    end
  end
  ...
  def create
    ...
    respond_to do |format|
      if get_session == @post.user_id && @post.save
  ...
  def update
    respond_to do |format|
      if get_session == params[:post][:user_id].to_i && @post.update(post_params)
